...
<%@page import="cyworld.model.Follow"%>
<html>
		...
		<div class="z-depth-3 myPageCard row">
			...
			<div class="col s4">
				<img class="profAvatar" height="230"
					...
					class="addressFont"><%=spritAddress[0]%></span> <br /> <br /> <a
					<%if (follow.getFollowInfo(loginUser.getUserID(), user.getUserID()) == false) {%>
					class="waves-effect waves-light btn follow-btn"
					href="FollowServlet?followID=<%=user.getUserID()%>&loginuser=<%=loginUser.getUserID()%>">Follow</a>
				<%
					} else {
				%>
				class="waves-effect waves-light btn Unfollow-btn"
				href="UnFollowServlet?followID=<%=user.getUserID()%>&loginuser=<%=loginUser.getUserID()%>">UNFollow</a>
				<%
					}
				%>
